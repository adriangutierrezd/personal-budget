...
import { getExpenses, updateExpense } from "@/lib/services/expenseService";
...
export default function ExpensesTable() {
  ...
  const columns: ColumnDef<Expense>[] = [
    ...
    },
    {
      header: "Acciones",
      accessorKey: "actions",
      cell: ({ row }) => <ExpensesTableActions handleUpdateExpense={handleUpdateExpense} categories={categories} row={row} />
    }
  ]

  return (
    <div className="container mx-auto py-10">
      {loading ? (<p>Cargando...</p>) : (<DataTable columns={columns} data={expenses} />)}
    </div>
  )
}
...
const ExpensesTableActions = ({ row, categories, handleUpdateExpense }: ExpensesTableActionsProps) => {

  const [isEditDialogOpen, setIsEditDialogOpen] = useState<boolean>(false)



  const editExpenseForm = useForm<z.infer<typeof editExpenseFormSchema>>({
  ...
  const onSubmit = async (values: z.infer<typeof editExpenseFormSchema>) => {
    console.log(values)
